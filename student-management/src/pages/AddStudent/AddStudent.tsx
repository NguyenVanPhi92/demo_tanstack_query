...
type FormStateType = Omit<Student, 'id'> | Student //tạo type cho form data

// khởi tạo giá trị ba đầu cho form data
...
export default function AddStudent() {
  ...
  const addMatch = useMatch('/students/add') // check url là add hay edit
  const isAddMode = Boolean(addMatch) // add or edit
  const { id } = useParams()
  const queryClient = useQueryClient() // dùng useQueryClient để tham chiếu đến query client

  // call api add student => trả về 1 promise
  ...
  // call api lấy ra student theo id
  const studentQuery = useQuery({
    queryKey: ['student', id],
    queryFn: () => getStudent(id as string),
    enabled: id !== undefined, // khi id khác undefine thì mới call api getStudent
    staleTime: 1000 * 10 // kiểm tra result trước đó đã quá 10s hay chưa nếu đã quá 10s rồi thì mới call lại api
  })
  ...
  // handle error khi call api và data gửi lên server
  ...
  // Dùng currying
  // keyof => name sẽ thuộc 1 trong những giá trị của FormStateType
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isAddMode) {
      // mutate là 1 async function xử lý bất đồng bộ
      addStudentMutation.mutate(formState, {
        // khi action api thành công
        onSuccess: () => {
